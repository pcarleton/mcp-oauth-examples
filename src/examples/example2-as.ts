// Val Town version - Example 2 Authorization Server (Path-aware with tenant)
...
// OAuth Authorization Server Metadata endpoint with tenant path
app.get('/.well-known/oauth-authorization-server/tenant1', (c) => {
  const baseUrl = `https://${c.req.header('host')}`;
  const issuer = `${baseUrl}/tenant1`; // Include tenant in issuer
...
// Health check
app.get('/health', (c) => {
  return c.json({ status: 'ok', server: 'mock-auth-server-tenant1' });
});

// For local development with Deno
if (import.meta.main) {
  const port = 3012;
  console.log(`Auth Server (tenant1) running at http://localhost:${port}`);
  console.log('\nEndpoints:');
  console.log(`  Metadata: http://localhost:${port}/.well-known/oauth-authorization-server/tenant1`);
