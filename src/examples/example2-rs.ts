// Val Town version - Example 2 Resource Server (Path-aware)
...
const AUTH_SERVER_URL = typeof Deno !== 'undefined' 
  ...
  : 'https://your-username-mcp-oauth-ex2-as.web.val.run';

const getServer = () => {
  const server = new McpServer({
    name: 'mcp-oauth-example-tenant1',
    version: '1.0.0',
  }, { 
    capabilities: { 
      tools: {} 
...
// OAuth Protected Resource metadata endpoint with tenant path
app.get('/.well-known/oauth-protected-resource/tenant1', (c) => {
  const resourceUrl = `https://${c.req.header('host')}`;
  
  return c.json({
    resource: resourceUrl,
    authorization_servers: [`${AUTH_SERVER_URL}/tenant1`] // Include tenant path
  });
});
...
app.post('/mcp', async (c) => {
  ...
  if (!authHeader || !authHeader.startsWith('Bearer ')) {
    const resourceMetadataUrl = `https://${c.req.header('host')}/.well-known/oauth-protected-resource/tenant1`;
...
if (import.meta.main) {
  const port = 3011;
  console.log(`Resource Server (tenant1) running at http://localhost:${port}`);
  console.log(`Auth Server expected at: ${AUTH_SERVER_URL}`);
  console.log('\nEndpoints:');
  console.log(`  MCP: http://localhost:${port}/mcp`);
  console.log(`  Metadata: http://localhost:${port}/.well-known/oauth-protected-resource/tenant1`);
